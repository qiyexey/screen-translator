...
一个安卓端实时屏幕翻译 App：**11 种翻译方式**、**11 家翻译引擎**、端侧 OCR 拍照翻译。
...
| ⌨️ **输入翻译**（v1.6.0） | 打字停顿 0.6 秒即自动翻译 | 输入框防抖 + 复用全部 11 家翻译引擎；悬浮球**长按**也能进入 |
...
| 📷 **拍照翻译**（v1.11.0） | 对着菜单/路牌/实物拍一张 | CameraX 取景 → **ML Kit 端侧 OCR**（本机识别、不依赖 GMS）→ 走任意文本引擎 |
...
翻译引擎支持 11 家（v1.4.0 起，源语言自动识别，目标语言可选）：
