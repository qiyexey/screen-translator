...
class LiveTranslateActivity : BaseActivity() {
     ...
     *
     * SeekBar 只支持整数，而间隔是毫秒（步进 100）、阈值是 2..50 —— 直接
     * 把真实值当 max 会让间隔滑杆从 0 到 5000，前面 90% 的行程都在 500ms 以内，
     * 手感很差。所以统一用"位置 → 值"的映射。
     */
    private fun setupSliders() {
        b.seekInterval.max = INTERVAL_STEPS
        b.seekInterval.progress = msToStep(App.prefs.liveIntervalMs)
        b.tvIntervalValue.text = "${App.prefs.liveIntervalMs}ms"
        b.seekInterval.setOnSeekBarChangeListener(object : SimpleSeekListener() {
        ...
        b.seekDiff.max = DIFF_MAX - DIFF_MIN
        b.seekDiff.progress = App.prefs.liveDiffThreshold - DIFF_MIN
        b.tvDiffValue.text = "${App.prefs.liveDiffThreshold}"
        b.seekDiff.setOnSeekBarChangeListener(object : SimpleSeekListener() {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        // 译文面板外观（v1.15.19）
        b.seekAlpha.max = ((1f - 0.15f) * 100).toInt()          // 15% ~ 100%
        b.seekAlpha.progress = ((App.prefs.liveOverlayAlpha - 0.15f) * 100).toInt()
        b.tvAlphaValue.text = "${(App.prefs.liveOverlayAlpha * 100).toInt()}%"
        b.seekAlpha.setOnSeekBarChangeListener(object : SimpleSeekListener() {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        b.seekPanelW.max = 100
        b.seekPanelW.progress = App.prefs.livePanelWPercent
        b.tvPanelWValue.text = fmt(App.prefs.livePanelWPercent, "%")
        b.seekPanelW.setOnSeekBarChangeListener(object : SimpleSeekListener() {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        b.seekPanelH.max = 40
        b.seekPanelH.progress = App.prefs.livePanelHPercent
        b.tvPanelHValue.text = fmt(App.prefs.livePanelHPercent, "%")
        b.seekPanelH.setOnSeekBarChangeListener(object : SimpleSeekListener() {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        b.seekTextScale.max = ((1.8f - 0.6f) * 100).toInt()     // 0.6x ~ 1.8x
        b.seekTextScale.progress = ((App.prefs.liveTextScale - 0.6f) * 100).toInt()
        b.tvTextScaleValue.text = String.format("%.2f×", App.prefs.liveTextScale)
        b.seekTextScale.setOnSeekBarChangeListener(object : SimpleSeekListener() {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        b.seekNudgeX.max = NUDGE_MAX * 2
        b.seekNudgeX.progress = App.prefs.liveNudgeX + NUDGE_MAX
        b.tvNudgeXValue.text = "${App.prefs.liveNudgeX}dp"
        b.seekNudgeX.setOnSeekBarChangeListener(object : SimpleSeekListener() {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        b.seekNudgeY.max = NUDGE_MAX * 2
        b.seekNudgeY.progress = App.prefs.liveNudgeY + NUDGE_MAX
        b.tvNudgeYValue.text = "${App.prefs.liveNudgeY}dp"
        b.seekNudgeY.setOnSeekBarChangeListener(object : SimpleSeekListener() {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
                val v = progress - NUDGE_MAX
    ...
    private fun refresh() {
        ...
        b.tvEngineStatus.setTextColor(resColor("md_success"))
