...
**模型加载（冷启动）实测 3.7~3.9 秒**（mmap 1.13GB + 建 context，连测两次），
首次从闪存读入可能更久。这就是设置页放「预加载」按钮的价值来源 ——
点一下把这几秒挪到用户主动等待的时候，而不是让第一句翻译莫名其妙地卡 4 秒。
「测试翻译」对本地引擎也会注明"首次包含模型加载时间"。
...
- **断点续传 + 自动重试**：`.part` 文件 + HTTP Range，失败自动从磁盘实际字节数续传
  （最多 6 次、退避 2/4/6/8/10s，连续 3 次零进展才放弃）。
  这是实测逼出来的：准备 Q6_K 档时，ModelScope 的 **HTTP/2 流在 1MB 处被服务端重置**
  （`HTTP/2 stream 1 reset by server (INTERNAL_ERROR)`），换成 HTTP/1.1 后又遇到一次
  **连接失败**（断在 164MB）。手机网络下这类中断是常态，把"再点一次"丢给用户
  等于让 5 分钟的下载必须有人守着。因此下载池固定走 HTTP/1.1
  （单条串行流用不上多路复用）+ 自动重试。
...
「多量化档可选」不能只验"文件存在"，所以把 Q6_K 也完整下下来跑了一遍：

| 量化档 | 文件大小（App 目录值 / 实测） | sha256 | 能否加载 | tg32 | 整句（13 句平均） |
|---|---|---|---|---|---|
| Q4_K_M | 1,133,080,448 / **一致** | ✅ 实测一致 | ✅ | **14.35 tok/s** | **0.83~1.03 s** |
| Q6_K | 1,474,785,120 / **一致** | ✅ 实测一致 | ✅ | 8.87 tok/s | 1.29~1.63 s |

- 两档的分词完全一致（手工官方格式都是 22 tokens、`<｜hy_User｜>`→120006、`<｜hy_Assistant｜>`→120007）
- Q6_K 的 13 句译文全部正确（含英/日/韩→中），没有垃圾输出
- Q6_K 生成慢约 38%（每权重字节更多、受内存带宽限制），与预期一致
  （注：Q6_K 那次测量时设备已连续工作较久，可能含热降频因素）
- Q8_0（1.91GB）只核对了大小与 sha256 常量，没有实际下载跑通

## 6. 多行输入的行结构保真（实测，结论是"不改"）
...
同一套 native 路径，把输入从 1 句加到 120 句（上下文用 App 默认的 2048）：

| 输入 | 输出上限 | 耗时 | 输出 | 结果 |
|---|---|---|---|---|
| 62 字 | 220 tok | 2.0 s | 51 字 | ✅ 正常中文 |
| 314 字 | 724 tok | 4.4 s | 255 字 | ✅ 正常中文 |
| 1259 字 | 1024 tok | 22.9 s | 1197 字 | ✅ 正常（但已很慢） |
| **3779 字** | 1024 tok | **89.5 s** | 4966 字 | ❌ **输出是英文续写，根本没翻译** |
| **7559 字** | 1024 tok | **125.7 s** | 4961 字 | ❌ **同样退化成续写** |

**问题**：输入一长就超出 2048 上下文，被截断后模型从"翻译"退化成"续写原文"，
而且用户要对着"正在翻译…"等 1.5~2 分钟。拍照翻译的「📄 全文」和长文章页面会踩到。

**修法**（`HyMtLocalTranslator`）：

1. **按行分块**，每块 ≤ **700 字**（取 1259 好 / 3779 坏之间的余量），
   逐块翻译再拼回。分块**不会增加总耗时**（耗时与总字数近似成正比），
   但避免了上下文截断导致的退化，也避免译文被 1024 token 上限截断。
   单行本身就超 700 字时按"需要几块"**平衡切分**（定长硬切会出现
   "700 + 1"这种末尾碎块，等于多发一次只翻一个字的请求）。
2. **总量超过 3000 字直接明确报错**，而不是硬跑：给出字数、说明原因、
   建议分段或临时切云端引擎。宁可拒绝，也不给一个要等几分钟还可能是英文的结果。

分块算法的等价性用 Python 精确复刻验证过：40 行×60 字 / 单行 2500 字 /
单行 701 字 / 混合空行与超长行 等用例下，**非换行字符零丢失、顺序不变、每块 ≤700**。
...
7. **长文本会被分块，硬切处译文多出换行**：单行超 700 字时会平衡切成多块分别翻译，
   拼接处会多一个换行（行边界分块不受影响）。语义无损，只是排版上多一行。
8. **App 自己报延迟**：设置页的本地模型区会显示
...
- [~] **装机实测（进行中）**：开发机没有设备控制档位，无法自行安装与点击，
      由用户手动安装并反馈。
  - 已证实可用：用户已装上并选中本地引擎，**模型下载 App 私有目录成功、能加载、能出译文**
    （反馈的截图是输入翻译页，报的是取消被当成失败 —— 见 §3.4，已修）
  - 待用户回报：设置页「跑一次基准」的 5 句毫秒数 + 「复制诊断信息」里的
    RSS 与指令集行（这两项我已做成 App 自报，见 §7）
