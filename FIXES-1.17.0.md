...
## 5. 多行输入的行结构保真（实测，结论是"不改"）

实时屏幕翻译走 OCR 时，是**把多行用 `\n` 拼成一段、一次请求**发出去的，
并且强依赖"译文行数 = 原行数"（对不上就退回整框重译，多花一次请求）。
所以专门测了提示词要不要强调换行 —— 4 类真实文本 × 3 次重复：

| 原文类型 | 官方默认提示词 | 强调"行数必须一致"的提示词 |
|---|---|---|
| 菜单 4 行 | 4 行 ✓✓✓ | 4 行 ✓✓✓ |
| 日语对话框 4 行 | 4 行 ✓✓✓ | 4 行 ✓✓✓ |
| 游戏对白 3 行 | 3 行 ✓✓✓ | 3 行 ✓✓✓（但多出尾随空格） |
| **折行段落 3 行** | **合并成 1 行** ✗✗✗ | **也合并成 1 行** ✗✗✗ |
| 合计 | 9/12 | 9/12 |

**结论：不改提示词。** 强调换行没有改善，还引入尾随空格噪音；
而唯一的失败案例是"一个句子被折成 3 个视觉行"，模型合并成一行在语义上是正确的。
代价只是那一类画面会多一次请求（`translateByOnDeviceOcr` 的整框重译兜底），
换来的是译文不被人为拆断。
...
6. **拍照翻译（点选逐行）在本地下是串行的**：那条路径用
   `Semaphore(MAX_PARALLEL)` 并发发请求（云端引擎受益），但本地推理被
   `HyMtRuntime` 的 mutex 串行化，所以 N 行大约 N × 1 秒。界面有
   "翻译中 i/N 行…" 进度，可用但明显比云端慢。
7. **App 自己报延迟**：设置页的本地模型区会显示
   "上次翻译：X ms（Y 行 / Z 字）"，"预加载 / 卸载"旁边就能看到 ——
   这是为了让没有设备控制权限的开发者也能拿到真机数字（用户念一行即可）。
